...
function buildMockService(overrides: Record<string, unknown> = {}) {
...
function buildAuthApp(mockOverrides?: Record<string, unknown>) {
  ...
  app.use('/api', router);
  app.use((_req: express.Request, res: express.Response) => res.status(404).json({ ok: false, mensaje: 'No encontrada.' }));
  app.use((err: Error, _req: express.Request, res: express.Response, _next: express.NextFunction) => res.status(500).json({ ok: false, mensaje: err?.message || 'Error interno.' }));
  return app;
}
