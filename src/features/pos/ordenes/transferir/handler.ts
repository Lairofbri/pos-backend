...
import { transferirItemsSchema } from './request.js';
import { transferirItems } from './service.js';

const manejarError = (res: Response, err: unknown) => {
  const e = err as { status?: number; mensaje?: string };
  if (e.status && e.mensaje) return error(res, e.mensaje, e.status);
  logger.error('Error no controlado en transferir items', { error: (err as Error).message, stack: (err as Error).stack });
...
export const handler = async (req: Request, res: Response) => {
  ...
  try {
    const resultado = await transferirItems({
