...
import { actualizarOrdenSchema } from './request.js';
import { actualizarOrden } from './service.js';

const manejarError = (res: Response, err: unknown) => {
  const e = err as { status?: number; mensaje?: string };
  if (e.status && e.mensaje) return error(res, e.mensaje, e.status);
  logger.error('Error no controlado en actualizar orden', { error: (err as Error).message, stack: (err as Error).stack });
...
export const handler = async (req: Request, res: Response) => {
  ...
  try {
    const totales = await actualizarOrden({
